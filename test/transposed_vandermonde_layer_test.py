...
def main_test():
    inputs = [[1, 2, 3], [3, 4, 5]]
    test_transposed_vandermonde(inputs, 2, False)
    test_transposed_vandermonde(inputs, 3, False)
    test_transposed_vandermonde(inputs, 8, False)
    test_transposed_vandermonde(inputs, 9, False, dtype=tf.float16)

    inputs = [[[1, 2], [3, 4]], [[5, 6], [7, 9]]]  # Like tensor
    test_transposed_vandermonde(inputs, 2, False)
    test_transposed_vandermonde(inputs, 4, False)
    test_transposed_vandermonde(inputs, 9, False, dtype=tf.float64)

    test_elm_with_transposed_vandermonde()

    print("Vandermonde test was succesful!")


def test_elm_with_transposed_vandermonde():
    inputs = [[1, 2, 3], [3, 4, 5]]
    elm = ELM()
    elm.add_dense_layer(10, input_dim=3)
    elm.add_layer(Vandermonde(35, transpose=True, use_bias=True))
    elm.add_dense_layer(20)
    elm.add_layer(Vandermonde(3, use_bias=False, transpose=True, drop_ones=True))
    elm.compile()
...
def test_transposed_vandermonde(inputs, neurons_number, do_printing,
                                dtype=None):
    # process inputs
    ...
    # init toeplitz layer
    vandermonde_layer = Vandermonde(neurons_number, normalize_weights_after_init=False, transpose=True, use_bias=False, activation=None, dtype=dtype)
    vandermonde_layer.build(inputs.shape)

    # calculate true and our results
    dense_matrix = np.vander(vandermonde_layer.get_parameters(), vandermonde_layer.units, increasing=True)
